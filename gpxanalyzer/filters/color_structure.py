...
class CSDescriptorExtractor:
    ...
    def calc_winsize(cell_size):
        '''
        Routine that mimics how window size is calculated in the libMPEG7
        '''
...
############################SUBROUTINES#################################################################################
    ...
    def __extract_bitstrings_no_check(self, cell):
        ...
        return out.transpose((1, 0, 2))\
            .reshape((mgr.cell_height, 
                      mgr.cell_width, 
                      out.shape[2] * 2))#[0:mgr.cell_height-gi.WINDOW_SIZE + 1, 0:mgr.cell_width - gi.WINDOW_SIZE + 1]
    ...
    def __extract_biststrings_greyscale_no_check(self,greyscale_cell):
        mgr = self.manager
        up_evt = cl.enqueue_copy(mgr.queue,self.quant_buffer, greyscale_cell, origin=(0,0),region=self.quant_buffer.shape)
        cl_evt = self.kernel_zero_out_image(mgr.queue, self.bitstring_buffer.shape, self.pixelwise_group_dims, self.bitstring_buffer)
    ...
    def extract_bitstrings_greyscale(self,greyscale_cell):
        self.__alloc_quant_buffer()
        self.__alloc_thresholds()
        self.__alloc_bitstring_buffer()
        self.extract_bitstrings_greyscale = self.__extract_biststrings_greyscale_no_check
        return self.__extract_biststrings_greyscale_no_check(greyscale_cell)
    ...
    def extract_descriptors(self, cell):
        bitstrings = self.extract_bitstrings(cell)
        return gi.bitstrings_to_descriptor_array_vector_extensions(bitstrings)
    
    def extract_descriptors_greyscale(self, greyscale_cell):
        bitstrings = self.extract_bitstrings_greyscale(greyscale_cell)
        return gi.bitstrings_to_descriptor_array_vector_extensions(bitstrings)
    
    def extract_bistatrings_descriptor(self, bitstrings, x, y):
        return gi.bitstrings_to_descriptor(bitstrings, x, y)
        
    def extract(self, tile):
        mgr = self.manager
        overlap = (gi.REGION_SIZE - 1, gi.REGION_SIZE - 1)
        out_size = (mgr.cell_height - overlap[0], mgr.cell_width - overlap[1], gi.BASE_QUANT_SPACE)
        return mgr.traverse_tile(tile, self.extract_descriptors, out_size, np.uint8, overlap)
    
    def extract_greyscale(self,tile):
        mgr = self.manager
        overlap = (gi.REGION_SIZE - 1, gi.REGION_SIZE - 1)
        out_size = (mgr.cell_height - overlap[0], mgr.cell_width - overlap[1], gi.BASE_QUANT_SPACE)
        return mgr.traverse_tile(tile, self.extract_descriptors_greyscale, out_size, np.uint8, overlap)
