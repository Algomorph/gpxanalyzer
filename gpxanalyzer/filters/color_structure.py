...
class ColorStructureDescriptorExtractor:
    ...
    def __init__(self, cl_manager):
        ...
        mgr = cl_manager
        log_area = math.log(float(mgr.cell_width*mgr.cell_height),2)
        ...
        self.program = cl.Program(cl_manager.context, 
                                  cs_cl_source
                                  ).build("-D WIDTH={0:d} -D HEIGHT={1:d} -D SUBSAMPLE={2:d} -D WINSIZE={3:d} -D BASE_QUANT_SPACE=256 -D GROUP_SIZE=8"
                                          .format(mod_width,mod_height,subsample,winsize))
