...
def set_up_retrieval(image_id, verbose = False):  
    ...
    if(verbose):
        ...
        % (width,height,str((tile_width, tile_height))))
    wt = width/tile_width
    ht = height/tile_height
    if (verbose):
        print "Tile dimensions: %d x %d" %(wt,ht)
    return wt, ht, tile_width, tile_height

def fetch_tile(output_folder, image_id, zoom, x, y, tile_width, tile_height, verbose = False):
    success = False
    while(not success):
        try:
            filename = "%04d-%04d.jpg"%(x,y)
            url = "%srender_image_region/%d/0/0/?c=1|0:255$FF0000&amp;m=g&amp;p=normal&amp;ia=0&amp;q=0.9&amp;zm=%d&amp;x=0&amp;y=0&amp;tile=0,%d,%d,%d,%d"%(base_jcb_url,image_id, zoom,x,y, tile_width, tile_height)
            if(verbose):
...
def fetch_tile_no_save(output_folder, image_id, zoom, x, y, tile_width, tile_height, verbose = False):
    filename = "%04d-%04d.jpg"%(x,y)
    url = "%srender_image_region/%d/0/0/?c=1|0:255$FF0000&amp;m=g&amp;p=normal&amp;ia=0&amp;q=0.9&amp;zm=%d&amp;x=0&amp;y=0&amp;tile=0,%d,%d,%d,%d"%(base_jcb_url,image_id, zoom,x,y, tile_width, tile_height)
    #print "Fetching tile %s from %s" % (filename,url)
    h = urllib2.urlopen(url)
...
def fetch_tile_wrapper(args):
    return fetch_tile_no_save(*args)
...
def is_tile_saved(x,y,folder):
    filename = "%04d-%04d.jpg"%(x,y)
    path = folder+os.path.sep+filename
    return os.path.isfile(path)

def check_what_is_done(wt, ht, output_folder,start_row):
    #check what's ready
    check_next_row = True
    x = 0; y = start_row
    for y in xrange(start_row,ht):
        for x in xrange(wt):
            if(not is_tile_saved(x,y,output_folder)):
...
if __name__ == '__main__':
    ...
    wt, ht, tile_width, tile_height = set_up_retrieval(image_id, verbose = True)
    
    start_x, start_y = check_what_is_done(wt,ht, output_folder,start_row)
    #exclusive bound
    end_y +=1
    ...
    for x_range in xrange(start_batch,n_batches):
        ...
        jobs = [gevent.spawn(fetch_tile, output_folder, image_id, 100, x, y, tile_width, tile_height, False) for x in xs]
        gevent.joinall(jobs)
        i_tile += batch_size
